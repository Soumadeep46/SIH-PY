...
def fetch_from_pinata(cid, jwt_token):
...
# Save uploaded images to Pinata and reload image list
if uploaded_image is not None:
    new_image_path = os.path.join(path, uploaded_image.name)
    with open(new_image_path, "wb") as f:
        f.write(uploaded_image.getbuffer())
    pinata_response = upload_to_pinata(new_image_path, PINATA_JWT_TOKEN)
    st.sidebar.success("Image uploaded successfully!")
    st.sidebar.write(f"IPFS CID: {pinata_response['IpfsHash']}")
